...
def generate_with_crewai(destination: str, days: int, search_results: str, budget: str, 
                        travel_style: str, interests: list, departure: str, selected_agents: list) -> str:
    try:
        # Set environment variable for Groq
        os.environ['GROQ_API_KEY'] = GROQ_API_KEY
        
        # Use direct Groq API calls instead of CrewAI LLM wrapper
        ...
        if "Research Agent" in selected_agents:
            research_prompt = f"""You are a Travel Researcher. Research comprehensive information about {destination}.
...
- Current events or festivals
            
Focus on {travel_style} travel style and interests: {', '.join(interests)}
            
Provide detailed research report."""
            
            research_result = call_groq_api(research_prompt)
            results.append(f"## 🔍 Research Report\n\n{research_result}")
        
        # Budget Agent
        if "Budget Agent" in selected_agents:
            budget_prompt = f"""You are a Budget Analyst. Calculate detailed budget breakdown for {days}-day trip to {destination}.
...
Provide realistic price ranges and money-saving tips."""
            
            budget_result = call_groq_api(budget_prompt)
            results.append(f"## 💰 Budget Analysis\n\n{budget_result}")
        
        # Itinerary Planner
        if "Itinerary Planner" in selected_agents:
            itinerary_prompt = f"""You are an Itinerary Planner. Create detailed {days}-day itinerary for {destination}.
...
Create day-by-day breakdown with:
- Morning activities
- Afternoon attractions
- Evening dining/entertainment
- Transportation between locations
            
Use **Day X:** format for each day."""
            
            itinerary_result = call_groq_api(itinerary_prompt)
            results.append(f"## 📅 Daily Itinerary\n\n{itinerary_result}")
        
        # Local Expert
        if "Local Expert" in selected_agents:
            expert_prompt = f"""You are a Local Expert for {destination}. Provide insider tips and hidden gems.
...
Focus on authentic, non-touristy recommendations."""
            
            expert_result = call_groq_api(expert_prompt)
            results.append(f"## 🏛️ Local Insider Tips\n\n{expert_result}")
        
        if not results:
            return "Please select at least one agent to generate your itinerary."
...
# Initialize session state and hardcode API keys
...
with col1:
    ...
    # Agent Selection
    st.markdown("### 🤖 Select AI Agents")
    col_agent1, col_agent2 = st.columns(2)
    
    with col_agent1:
        use_crewai = st.checkbox("🚀 Use CrewAI Multi-Agent System", help="Enable specialized AI agents")
        
    if use_crewai:
        with col_agent2:
            selected_agents = st.multiselect(
                "Choose Agents:",
                ["Research Agent", "Budget Agent", "Itinerary Planner", "Local Expert"],
                default=["Research Agent", "Itinerary Planner"]
            )
    else:
        selected_agents = []
...
if st.button("🎯 Generate My Perfect Itinerary", type="primary"):
    if not destination:
        ...
        try:
            ...
            # Step 2: Generate with selected method
            if use_crewai and selected_agents:
                status_text.text("🤖 CrewAI agents working together...")
                progress_bar.progress(66)
                
                itinerary = generate_with_crewai(
                    destination, num_days, search_results, budget, 
                    travel_style, interests, departure, selected_agents
                )
            else:
